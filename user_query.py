...
def _peers_changed_since_check():
    ...
    try:
        _self_ip = LocalConfig.load().tailscale_ip
        current = set(p["ip"] for p in get_online_peers()) - {_self_ip}
...
def send_query(query, local: LocalConfig, session_manager, daemon_port=65433, on_token=None):
    ...
    # Refresh peer baseline so the next warm query doesn't re-check
    # immediately. Self is excluded: get_online_peers() reports other
    # machines only, so including it here would make every subsequent
    # check see this machine as having left.

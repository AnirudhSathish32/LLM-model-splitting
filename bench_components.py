...
from user_query import UserQuery, send_query, get_pipeline_info, get_last_timing_records
...
def experiment_cost_model(local, sm, model_name, repeats, tokens):
    ...
    for rep in range(repeats):
        ...
        try:
            ...
            # Distributed runs record inside the daemon process and send
            # the records back over the stream; single-node records here.
            collected.extend(get_last_timing_records() or telemetry.rows())
            telemetry.clear()
    ...
    telemetry.disable()
    rows = [r for r in collected if r.get("phase") == "decode"]
    if not rows:
...
def experiment_fairness(local, sm, model_name, n_concurrent, tokens):
    ...
    def fire(i):
        ...
        try:
            send_query(q, local, sm)
            recs = get_last_timing_records() or telemetry.rows()
            with collect_lock:
    ...
    # Order by arrival. Records from the daemon have no wall_time, so
    # fall back to token_index within each session.
    for i, r in enumerate(collected):
        r.setdefault("wall_time", i)
    rows = sorted(collected, key=lambda r: r["wall_time"])
    if not rows:

...
def validate_all_layers(full_outputs, split_outputs, tolerance=1e-2):
    ...
    for idx in sorted(full_outputs.keys()):
        ...
        print(f"{idx:<8} {max_diff:<14.6f} {mean_diff:<14.6f} {cos_sim:<12.6f} {'✓' if match else '✗'}")
...
if __name__ == "__main__":
    ...
    model, inputs, tokenizer = machine_a.setup_model_a(stopping_layer, model_path)
    split_monitor = ResourceMonitor()
    split_monitor.start()
    split_start = time.time()
    response, all_layer_outputs = machine_a.run_machine_a(tokens_to_generate, stopping_layer, tokenizer, conn)
